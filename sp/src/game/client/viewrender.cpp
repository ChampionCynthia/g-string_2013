...
void CViewRender::RenderView( const CViewSetup &view, int nClearFlags, int whatToDraw )
{
	...
	else
	{
		...
		if ( whatToDraw & RENDERVIEW_DRAWHUD && !bBuildingCubemaps )
		{
			...
			DrawVGUILayer( CHud::HUDRENDERSTAGE_PRE_HDR, view, NULL );
			render->PopView( GetFrustum() );
		...
		{
			render->Push2DView( view, 0, saveRenderTarget, GetFrustum() );
			DrawVGUILayer( CHud::HUDRENDERSTAGE_PRE_BARS, view, NULL );
			render->PopView( GetFrustum() );
			UpdateScreenEffectTexture( 0, view.x, view.y, view.width, view.height );
		}
	...
	// GSTRINGMIGRATION
	if ( whatToDraw & RENDERVIEW_DRAWHUD )
		DrawVGUILayer( CHud::HUDRENDERSTAGE_DEFAULT_HUD, view, saveRenderTarget );
	// END GSTRINGMIGRATION
